...
export class ResourceCacheService {
  ...
  public evictResource(key: CacheKey): void {
    ...
    const resourceName = key.url.replace(`${ UrlUtils.identifyApiSource(key.url) }/`, '').split('/')[0];
    if (!resourceName) {
      return;
    }
    const evictedCache = [];
    for (const cacheKey of this.cacheMap.keys()) {
      if (cacheKey.startsWith(`url=${ UrlUtils.identifyApiSource(key.url) }/${ resourceName }`)) {
        evictedCache.push({
          key: cacheKey
        });
